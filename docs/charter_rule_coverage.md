...
| VR10 | No connected kVA | `checks/loads.py` | `NoConnectedKVA` | None | Starts from all sections and left-joins load totals, so sections with no load rows are included. Uses kVA directly when available, derives apparent power from `sqrt(kW^2 + kVAR^2)` when both are available, and documents proxy use when only kW or only kVAR exists. |
